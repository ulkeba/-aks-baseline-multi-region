...
1. Create two hubs, and two spokes that will be home to the AKS clusters and its adjacent resources and then enroll the spokes into the hubs.
   ...
   >
   > The networking team has decided that `10.200.[0-9].0` will be where all regional hubs are homed on their organization's network space. The `eastus2` and `centralus` hubs (created below) will be `10.200.3.0/24` and `10.200.4.0/24` respectively.
   >
